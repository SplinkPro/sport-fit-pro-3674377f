        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-foreground">{users.length} Users</span>
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/40 text-left">
                  ...
                  <th className="px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Provider</th>
                  <th className="px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Joined</th>
                  <th className="px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Last Sign-in</th>
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-muted/40 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {u.avatar_url ? (
                            <img src={u.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover ring-2 ring-border" />
                          ) : (
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-muted text-muted-foreground capitalize">
                          {u.provider}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        {u.confirmed_at ? (
                          <span className="flex items-center gap-1 text-success text-xs font-semibold">
                            <span className="w-1.5 h-1.5 rounded-full bg-success inline-block" /> Verified
                          </span>
                        ) : (
                          <span className="flex items-center gap-1 text-warning text-xs font-semibold">
                            <span className="w-1.5 h-1.5 rounded-full bg-warning inline-block" /> Pending
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        {u.id === user?.id ? (
                          ...
                          <div className="relative">
                            ...
                            {openRoleDropdown === u.id && (
                              <div className="absolute z-20 top-full mt-1 left-0 bg-popover border border-border rounded-xl shadow-lg overflow-hidden w-36">
                                {ALL_ROLES.map(r => (
                                  <button
                                    key={r}
                                    onClick={() => assignRole(u.id, r)}
                                    className={`w-full flex items-center gap-2 px-3 py-2.5 text-xs font-semibold hover:bg-muted transition-colors ${r === currentRole ? "bg-muted" : ""}`}
                                  >
                                    <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full ${ROLE_COLORS[r]}`}>
                                      {ROLE_ICONS[r]} {r}
                                    </span>
                                    {r === currentRole && <span className="text-muted-foreground ml-auto">✓</span>}
                                  </button>
                                ))}
                              </div>
                            )}
